  ...
  <div class="mypage">
    ...
    <section class="mypage-card">
      ...
      <div class="supply-stat-grid">
        ...
        <div class="supply-stat-card">
          ...
          <div class="ssc-info">
            <span class="ssc-label">받은 요청</span>
        ...
        <div class="supply-stat-card">
          ...
          <div class="ssc-info">
            <span class="ssc-label">확인 대기</span>
        ...
        <div class="supply-stat-card">
          ...
          <div class="ssc-info">
            <span class="ssc-label">납품 가능</span>
    ...
    <div class="two-col-grid">
      <section class="mypage-card">
        <div class="card-header">
          <h2>최근 문의</h2>
          <RouterLink class="link-more" :to="{ name: 'inquiries' }">전체 보기 ›</RouterLink>
        </div>
        <div v-if="recentInquiries.length === 0" class="empty-state">
          ...
          <RouterLink class="btn-go" :to="{ name: 'inquiries' }">문의 관리 바로가기</RouterLink>
        </div>
...
const rejectedStatuses = new Set(["rejected", "unavailable"]);
const pendingCount = computed(() => inquiries.value.filter(i => pendingStatuses.has(i.status)).length);
const acceptedCount = computed(() => inquiries.value.filter(i => availableStatuses.has(i.status)).length);
...
onMounted(async () => {
  try {
    ...
    inquiries.value = filterInquiriesForUser(inquiryResult, authState.user);
...
function statusLabel(s) {
  if (pendingStatuses.has(s)) return "확인 대기";
  if (availableStatuses.has(s)) return "납품 가능";
  if (rejectedStatuses.has(s)) return "거절";
  if (s === "need_more_info") return "추가 확인 필요";
  return "상태 확인 중";
}
function statusClass(s) {
  if (pendingStatuses.has(s)) return "status-pending";
  if (availableStatuses.has(s)) return "status-accepted";
  if (rejectedStatuses.has(s)) return "status-rejected";
  return "";
}
