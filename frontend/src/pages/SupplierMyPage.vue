<template>
  <div class="mypage">
    <div class="page-header">
      ...
      <RouterLink class="btn-outline" to="/supplier/profile">프로필 편집</RouterLink>
    </div>

    <!-- 빠른 이동 -->
    <div class="quick-links">
      <RouterLink class="quick-card" to="/supplier/dashboard">
      ...
      <RouterLink class="quick-card" to="/supplier/profile">
        <span class="ql-icon">📦</span>
        <div>
      ...
      <RouterLink class="quick-card" to="/inquiries">
        <span class="ql-icon">💬</span>
        <div>
    ...
    <section class="mypage-card">
      ...
      <div class="stat-row">
        <div class="stat-item" v-for="s in supplierStats" :key="s.label">
    ...
    <section class="mypage-card">
      <div class="card-header">
        <h2>최근 문의</h2>
        <RouterLink class="link-more" to="/supplier/dashboard">전체 보기</RouterLink>
      </div>
      <div v-if="recentInquiries.length === 0" class="empty-state">접수된 문의가 없습니다.</div>
...
const displayName = computed(() => authState.user?.username ?? authState.user?.email ?? "공급사");
...
const supplierStats = computed(() => [
  { label: "등록 자재", value: `${materials.value.length}종` },
  { label: "전체 문의", value: `${inquiries.value.length}건` },
  { label: "검토 중", value: `${inquiries.value.filter((i) => i.status === "pending").length}건` },
  { label: "수락", value: `${inquiries.value.filter((i) => i.status === "accepted").length}건` },
]);
...
.quick-card:hover { border-color: #1559e8; }
.ql-icon { font-size: 24px; }
.quick-card strong { font-size: 14px; color: #102a56; font-weight: 700; display: block; margin-bottom: 2px; }
.quick-card p { font-size: 12px; color: #71809a; margin: 0; }
...
.s-label { font-size: 12px; color: #71809a; }
.s-value { font-size: 26px; font-weight: 800; color: #102a56; }
